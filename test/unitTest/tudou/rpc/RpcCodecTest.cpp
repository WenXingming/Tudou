...
TEST_F(RpcCodecTest, EncodesAndDecodesValidFrameSuccessfully) {
    uint64_t seq = 0x123456789ABCDEF0ULL;
    std::string meta = "UserService.Login";
    std::string body = "{\n  \"username\": \"alice\"\n}";

    // 编码写入缓冲区
    RpcCodec::encode(&buffer, RpcMessageType::Request, seq, meta, body);
    ...
    std::string outMeta;
    std::string outBody;
    RpcCodec::DecodeResult result = RpcCodec::decode(&buffer, outHeader, outMeta, outBody);

    ASSERT_EQ(result, RpcCodec::DecodeResult::Success);
...
TEST_F(RpcCodecTest, HandlesHalfPackSuspensionCorrectly) {
    uint64_t seq = 8888;
    ...
    // 制作一个完整的二进制包
    Buffer tempBuffer;
    RpcCodec::encode(&tempBuffer, RpcMessageType::Response, seq, meta, body);
    ...
    EXPECT_EQ(RpcCodec::decode(&buffer, outHeader, outMeta, outBody), RpcCodec::DecodeResult::Empty);
    ...
    EXPECT_EQ(RpcCodec::decode(&buffer, outHeader, outMeta, outBody), RpcCodec::DecodeResult::HalfPack);
    EXPECT_EQ(buffer.readable_bytes(), 22); // 读指针仍不能发生推进

    // C. 阶段三：将剩余字节流倾泻写入（收全数据）
    buffer.write_to_buffer(rawData.data() + 22, rawData.size() - 22);
    EXPECT_EQ(RpcCodec::decode(&buffer, outHeader, outMeta, outBody), RpcCodec::DecodeResult::Success);
...
TEST_F(RpcCodecTest, DemuxesStickyPackagesCorrectly) {
    // 编码包 1
    RpcCodec::encode(&buffer, RpcMessageType::Request, 100, "Method1", "Body1");
    // 直接粘着编码包 2
    RpcCodec::encode(&buffer, RpcMessageType::Response, 200, "Method2", "Body2_Longer");
    ...
    // 解析包 1
    ASSERT_EQ(RpcCodec::decode(&buffer, header1, meta1, body1), RpcCodec::DecodeResult::Success);
    EXPECT_EQ(header1.sequenceId, 100);
    EXPECT_EQ(meta1, "Method1");
    EXPECT_EQ(body1, "Body1");

    // 解析包 2
    ASSERT_EQ(RpcCodec::decode(&buffer, header2, meta2, body2), RpcCodec::DecodeResult::Success);
...
TEST_F(RpcCodecTest, ErrorsOutOnMagicMismatch) {
    ...
    EXPECT_EQ(RpcCodec::decode(&buffer, outHeader, outMeta, outBody), RpcCodec::DecodeResult::Error);
