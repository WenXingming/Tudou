 ...
 *
 * - 封装解析 HTTP 报文的类，基于 llhttp 库实现。解析 HTTP 请求报文，结果存储在 HttpRequest 对象中。
 *
 * - 对外接口包括：
 *   - parse(): 解析传入的数据，返回是否成功解析。nparsed 输出实际解析的字节数。只要无语法错误即视为消费了全部输入数据即 len 字节
 *   - is_complete(): 检查报文是否完整解析，判断是否收到了完整的一个 HTTP 请求
 *   - get_request(): 获取解析后的 HttpRequest 对象，包含请求的各个部分
