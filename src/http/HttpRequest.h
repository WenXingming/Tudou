 ...
 *   - 头部字段
 *   - 消息体
 *   - path 和 query 是 URL 的拆分部分。例如，URL 为 /search?q=example 时，path 为 /search，query 为 q=example。
 * - 提供方法设置和获取这些部分: getters 和 setters。
 * - 提供清空请求报文内容的方法: clear()。
 * - HttpRequest 无需有 package_to_string() 方法，因为请求报文通常由客户端生成并发送。服务器端主要负责解析和处理请求报文，然后HttpResponse 用于生成响应报文。
 */
