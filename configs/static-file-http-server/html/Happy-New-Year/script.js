function init() {
    ...
    // Mobile tall screens: avoid cropping important content (e.g. "Happy New Year")
    // by using 'meet' instead of 'slice'. Desktop keeps the full-bleed look.
    var isMobile = window.matchMedia && window.matchMedia('(max-width: 768px)').matches;
    var isTallScreen = isMobile && (window.innerHeight / Math.max(1, window.innerWidth) > 1.25);
    if (svg && svg.setAttribute) {
        svg.setAttribute('preserveAspectRatio', isTallScreen ? 'xMidYMid meet' : 'xMidYMid slice');
    }

    // Slow down the Happy New Year scene animations.
    // 1.0 = normal speed, smaller = slower.
    var sceneTimeScale = 0.5;
    ...
    // Larger factor = faster falling (shorter duration). Mobile needs a stronger boost.
    var snowSpeedFactor = isMobile ? 3.2 : 2.0;
    for (var i = 0; i < snow.length; i++) {
        rand = Math.floor(Math.random() * 2);
        var offset = snow[i].getBoundingClientRect();
        // On mobile, DOM rects are unreliable for SVG distribution; bias flakes to an "upper" group.
        var isUpperArea = !isMobile || (Math.random() < 0.82);
        var localSpeedFactor = isMobile ? (snowSpeedFactor * (isUpperArea ? 1.25 : 0.70)) : snowSpeedFactor;
        var localOpacity = isMobile ? (isUpperArea ? 1 : 0.12) : 1;
        var startY = isMobile
            ? -(Math.random() * (isUpperArea ? 980 : 520) + 60)
            : -(offset.top + offset.width + 100);
        TweenMax.set(snow[i], {
            y: startY,
            opacity: localOpacity
        });
        TweenMax.to(snow[i], (Math.random() * 50 + 50) / localSpeedFactor, {
            y: 920,
            repeat: -1,
            delay: -Math.random() * (isMobile ? (isUpperArea ? 55 : 180) : 100)
        });
    }
    ...
    if (typeof TweenMax !== "undefined" && typeof TweenMax.globalTimeScale === "function") {
        TweenMax.globalTimeScale(sceneTimeScale);
    }
    ...
    // Skip the gift box step: go straight to the Happy New Year scene.
    startScene();
