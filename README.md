...
这些结果表明该服务器在单 Reactor 模式下能够高效地处理大量并发请求，具有较低的响应时间和较高的吞吐量。作为对比，测试了 muduo TcpServer 的单线程性能（未含 HTTP 协议解析），结果如下：
...
这些结果表明该服务器在多 Reactor 模式下能够显著提升并发处理能力，响应时间进一步降低，吞吐量大幅提升，展示了良好的扩展性和高性能。作为对比，测试了 muduo TcpServer 的多线程性能（未含 HTTP 协议解析），结果如下：

```bash
# 多线程 hello world TcpServer 测试
