...
Channel 构造时 `events_` 是 `kNoneEvent`（0），`update_in_register()` 把这个"无事件兴趣"状态同步到 Poller。这相当于告诉 epoll："我有这个 fd 了，但现在不关注任何事件。"
...
`disable_all()` 把 `events_` 设为 `kNoneEvent`，然后 `epoll_ctl(MOD)` 更新——fd 还在 epoll 里，只是不关注任何事件了。这相当于"暂停监听"，之后可以再 `enable_reading()` 恢复。
...
**执行阶段**：遍历 `expiredTimers`，逐个 `timer->run()`。执行后重新检查 `timersById_.find(timer->id())`——因为回调中可能调用了 `erase_timer` 取消了自己或其他定时器。一次性定时器从 `timersById_` 中删除；重复定时器调用 `restart(now)` 推进到期时间，重新 `push` 回 `expireHeap_`。
...
- **Timer**：空回调不崩溃、周期定时器 restart 推进时间、TimerId 排序和相等。
