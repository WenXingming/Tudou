...
- `Buffer::kInitialSize_ = 1024`
- `Buffer::kCheapPrepend_ = 8`
...
### 3.1.1 为什么要保留 prepend 区？

这里的 `kCheapPrepend_ = 8` 不是为了“显得和 muduo 一样”，而是为了让缓冲区头部始终保留一小段安全余量，并在空间回收时有一个统一的复位基准点。

当前仓库虽然没有暴露专门的 prepend 写接口，但这块区域依然有两个现实作用：

1. 让可读区在逻辑上不必从 0 开始，布局更稳定
2. 在 `make_space()` 时，可把已有可读数据整体搬回 `kCheapPrepend_` 之后，复用前部空洞
